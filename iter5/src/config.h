...
// Iteration: 1 (extended Iter 2, 3, 4, 5)
...
namespace Cfg {
	...
	// ── Payload Bay / Door Region (Iter 2, fixed Iter 5) ────
	...
	constexpr float  PBAY_Z_START = CABIN_HUMP_Z - CABIN_HUMP_LENGTH * 0.5f; // cabin aft edge
	...
	constexpr float  SEAT_BACK_H = 0.12f;    // backrest height (shortened for lower ceiling)
	...
	// The full door region covers the entire cabin hump roof
	// (PBAY_Z_START → PBAY_Z_START + PBAY_LENGTH) but is split into
	// an aft segment and a forward segment to clear the overhead panel.
	constexpr float  DOOR_GAP_MARGIN = 0.0f;    // doors abut overhead (different Y, no z-fight)
	...
	// Aft door segment (behind overhead panel)
	constexpr float  DOOR_AFT_Z_START = PBAY_Z_START;                         // 2.35
	constexpr float  DOOR_AFT_LENGTH = DOOR_GAP_Z_AFT - DOOR_AFT_Z_START;   // ~0.605

	// Forward door segment (ahead of overhead panel)
	constexpr float  DOOR_FWD_Z_START = DOOR_GAP_Z_FWD;                                     // ~3.245
	constexpr float  DOOR_FWD_LENGTH = (PBAY_Z_START + PBAY_LENGTH) - DOOR_FWD_Z_START;    // ~0.255
	...
	constexpr int    DOOR_AFT_RIB_COUNT = 4;   // ribs on aft door
	constexpr int    DOOR_FWD_RIB_COUNT = 2;   // ribs on fwd door
