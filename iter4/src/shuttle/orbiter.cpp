...
// ---------------------------------------------------------------------------
// drawPayloadBayOutline: seam lines (only when doors are closed).
// ---------------------------------------------------------------------------
static void drawPayloadBayOutline() {
    float halfW = Cfg::PBAY_WIDTH * 0.5f;
    float z0 = Cfg::PBAY_Z_START;
    float z1 = z0 + Cfg::PBAY_LENGTH;
    float y = Cfg::FUS_RADIUS_AFT - 0.01f;
    float b = Cfg::PBAY_BORDER;

    matPayloadLine();

    float f0[3] = { -halfW, y, z1 };     float f1[3] = { halfW, y, z1 };
    float f2[3] = { halfW, y, z1 - b }; float f3[3] = { -halfW, y, z1 - b };
    DrawQuad(f0, f1, f2, f3);

    float a0[3] = { -halfW, y, z0 + b }; float a1[3] = { halfW, y, z0 + b };
    float a2[3] = { halfW, y, z0 };     float a3[3] = { -halfW, y, z0 };
    DrawQuad(a0, a1, a2, a3);

    float l0[3] = { -halfW,     y, z0 }; float l1[3] = { -halfW + b, y, z0 };
    float l2[3] = { -halfW + b, y, z1 }; float l3[3] = { -halfW,     y, z1 };
    DrawQuad(l0, l1, l2, l3);

    float r0[3] = { halfW - b, y, z0 }; float r1[3] = { halfW,     y, z0 };
    float r2[3] = { halfW,     y, z1 }; float r3[3] = { halfW - b, y, z1 };
    DrawQuad(r0, r1, r2, r3);

    float c0[3] = { -b * 0.5f, y, z0 }; float c1[3] = { b * 0.5f, y, z0 };
    float c2[3] = { b * 0.5f, y, z1 }; float c3[3] = { -b * 0.5f, y, z1 };
...
void drawOrbiter() {
    ...
    // Payload bay outline only when doors are fully closed
    if (gDoorAngle < 1.0f) {
        glPushMatrix(); drawPayloadBayOutline(); glPopMatrix();
    }

    // Payload bay doors (always drawn — animated)
    glPushMatrix(); drawPayloadBayDoors(gDoorAngle); glPopMatrix();

    // Payload bay interior structure (always visible — seamlessly
    // integrated; when doors are closed the structure sits behind them)
    glPushMatrix(); drawPayloadBayInterior(); glPopMatrix();
