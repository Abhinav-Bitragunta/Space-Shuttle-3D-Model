...
// Iteration: 1 (Iter 2: H, Iter 3: S/L, Iter 4: F/O/WASD + interior)
...
static void updateFPS() {
    ...
    if (elapsed > 100) {
        ...
        snprintf(title, sizeof(title),
            "%s  |  Highlight: %s  |  Doors: %s  |  Light: %s  |  Shading: %s  |  FPS: %.0f",
...
static void display() {
    ...
    // Iter 4: wider FOV and closer near-clip for interior
...
static void keyboard(unsigned char key, int /*x*/, int /*y*/) {
    ...
    switch (key) {
    case 27:
        // In interior mode, Esc returns to exterior first
        if (interior) {
            gCamera.toggleMode();
        }
        else {
    ...
    case 'f': case 'F':  // Iter 4: toggle camera mode
        gCamera.toggleMode();
        break;

    case 'o': case 'O':  // Iter 4: toggle door animation
        toggleDoorAnimation();
        break;
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Space Shuttle — Iteration 4");
    ...
    glutPassiveMotionFunc(passiveMotion);  // Iter 4: interior mouse-look
